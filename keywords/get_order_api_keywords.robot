...
Send Request To Get Product Order
    ${order_id_part}    Replace String    ${api_path.single_order}    {order_id}    ${order_id}
    ${response}    Send Get Request With Header    ${api_coffee_shop['url']}    ${order_id_part}    ${headers}
    Set Test Variable    ${response}    ${response}

Verify Product Order Is A Success
    Status Should Be    ${order_list.success.code}    ${response}
...
Verify Product Order Is A Failed
    Status Should Be    ${order_list.failed.code}    ${response}
